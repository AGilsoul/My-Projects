...
void ConvNet::addConvLayer(int numKernels, int kernelDim, bool padding, bool pooling, const string& poolType) {
    ...
    // create new layer and add to the network
    if (convLayers.empty()) {
...
int ConvNet::predict(tensor data) {
    // get result of forward propagation
    auto res = propagate(data);
    // find max index, return that index
    int maxIndex = -1;
    double maxVal = 0;
...
vector<double> ConvNet::propagate(tensor dataIn) {
    // get result from every convolution layer, use as input for the next layer
    for (auto & convLayer : convLayers) {
        dataIn = convLayer->propagate(dataIn);
    }
    // get result from convolution layers, propagate through flat dense layers
...
matrix ConvNet::oneHotEncode(vector<int> labels) {
    // gets the number of labels, then encodes labels
...
double ConvNet::eval(vector<tensor> testData, vector<int> testLabels) {
    // gets decimal of number of correct predictions / number of test points
    double numCorrect = 0;
