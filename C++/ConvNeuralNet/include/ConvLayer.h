...
public:
    ...
    void update();
    /**
     * Set the learning rate for the layer (no different than other layers as of yet)
     * @param lr learning rate to be set
     */
...
public:
    ...
    /**
     * Assigns class variables based on external input, called by model loading methods in ConvNet.h
     * @param kernels weights for each kernel in the layer
     * @param numKernels number of kernels in the layer
     * @param kernelDim dimension of each kernel in the layer
     * @param inputChannels number of input channels
     * @param inputDim dimension of the input
...
private:
    /**
     * sets up rng for weights, vector dimensions, etc.
     */
    void initLayer();
    /**
