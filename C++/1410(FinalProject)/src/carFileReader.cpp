...
carFileReader::carFileReader() {
    ifstream fin("CarDatabase.csv", ios::in);
    int carCounter = 0;
    ...
    while (!fin.eof()) {
        ...
        percentLoaded = loadBar(carCounter);
        //cout << setw(64) << "Cars Loaded: " << carCounter << endl;
        //cout << setw(28) << "[" << (int)(percentLoaded) << "%] <" << loadCount << setw(49 - loadCount.length() - 1) << ">" << endl;
    }
...
//adds hyphens to loading bar string for display
double carFileReader::loadBar(int currentCount) {
    int total = 423;
    percentLoaded += 0.237;
    if (currentCount % 9 == 0) {
        loadCount += "-";
